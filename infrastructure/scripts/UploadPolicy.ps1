...
if ($FunctionAppName) {
  Write-Host "-------------------------------------"
  Write-Host "Step 1: Check Function App Key (Informational Only)"
  Write-Host "-------------------------------------"
  Write-Host "Bicep should manage the Function Key in APIM ('function-key' named value)."
  Write-Host "Skipping legacy key update logic."
}
