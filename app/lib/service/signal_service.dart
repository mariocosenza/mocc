...
final signalRefreshProvider = StreamProvider<String>((ref) {
...
class SignalService {
  ...
  Future<Map<String, dynamic>?> _negotiateWithRetry(
    String userId,
    String apiToken,
  ) async {
    int retries = 0;
    ...
    while (retries < maxRetries) {
      ...
      if (retries < maxRetries) {
        ...
        final delaySeconds =
            (2 * retries) + (DateTime.now().millisecond % 1000) / 1000.0;
      ...
      try {
        ...
        if (data is Map) {
          if (data['type'] == 'refresh') {
            _refreshController.add(DateTime.now().toIso8601String());
          }
        }
      } catch (e) {
